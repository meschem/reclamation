...
initialMapPoiObjects = [
    obj_boss_obelisk,
    obj_elite_obelisk,
    obj_dest_vein_iron,
    obj_room_exit,
    obj_merchant_merger_zone,
    obj_waypoint
]
...
if (layer_exists(layerId)) {
    ...
    playerCollisionList = [obj_pit, obj_player_clip, obj_doodad, obj_destructible, tileId]	
} else {
    playerCollisionList = [obj_pit, obj_player_clip, obj_doodad, obj_destructible]	
} 
...
create_instance(obj_swarm_controller)
// Dormancy controller created on first map load only in initCombat()
...
addActiveWaypoint = function(_waypoint) {
    array_push(activeWaypoints, _waypoint)
    
    array_sort(activeWaypoints, function(_a, _b) {
        return (_a.index > _b.index)
    })
    
    with (obj_run_controller) {
        if (!array_contains(unlockedWaypoints, _waypoint.waypointName)) {
            array_push(unlockedWaypoints, _waypoint.waypointName)
        }
    }
}
...
initCombat = function(_singleRoom = false) {
    create_toaster("Init Combat")
	showTimeDisplay = true
	timerActive = true
	runIsActive = true
