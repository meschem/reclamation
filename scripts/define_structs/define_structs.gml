...
///@description                                         Stored instances for a map, used for loading and unloading
///@param {id.Instance} _inst                           Instance to store
///@param {array<string>} _vars                         Array of variables referenced as strings to store
function storedInstance(_inst, _vars = []) constructor {
    object = _inst.object_index
    spawnX = _inst.x
    spawnY = _inst.y
    spawnDepth = _inst.depth
    ...
    spawn = function() {
        var _inst = instance_create_depth(
            spawnX, spawnY, spawnDepth, object
        )
        
        for (var i = 0; i < array_length(varStructs); i++) {
            variable_instance_set(_inst, varStructs[i].name, varStructs[i].value)
        }
    }
}

///@description                                         A group of swarms used for spawning
///@param {array<struct.swarmGroup>} _swarmGroups       Swarm Groups                      
///@param {real} _difficulty                            Difficulty assigned to collection
///@param {array<real>} _biomes                         Uses enum enumBiomes to assign a biome that they spawn in
function swarmGroupCollection(_swarmGroups, _difficulty, _biomes = []) constructor {
    swarmGroups = _swarmGroups
    difficulty = _difficulty
    biomes = _biomes
    
    ///@description                     Spawns the groups
    ///@return {array<id.Instance>}
    spawn = function() {
        var _baddies = []
         
        for (var j = 0; j < array_length(swarmGroups); j++) {
            var _group = swarmGroups[j] 
            var _roll = random(1)
            
            if (_roll < _group.chanceToSpawn) {
                var _spawned = spawn_baddie_swarm(_group.baddie, _group.count)
                
                array_concat(_baddies, _spawned)
            }
        }
    ...
    ///@description                     Spawns an elite pack based off a randomly selected baddie
    ///@return {array<id.Instance>}
    spawnElite = function() {
        var _baddies = []
        var _spawned = false
        ...
        for (var j = 0; j < array_length(swarmGroups); j++) {
            ...
            if (_roll < _group.chanceToSpawn) {
                var _elite = spawn_baddie(_group.baddie, 1, 1, false)
                _spawned = true
                
                create_elite_health_bar(_elite[0])
		        enhance_baddie(baddieSpecialTypes.elite, _elite[0]) 
                _baddies = apply_random_elite_buff(_elite[0])
        ...
        if (!_spawned) {
            create_toaster("Elite baddie not found and spawned", errorLevels.error)
        }
...
///@description                         A group of baddies used for swarm spawning
///@param {asset.GMObject} _baddie      Baddie to spawn
///@param {real} _count                 Amount to spawn, later multiplied by global spawn multiplier
///@param {real} _chanceToSpawn         Percent chance to spawn this baddie 
function swarmGroup(_baddie, _count, _chanceToSpawn = 1) constructor {
    baddie = _baddie
    count = _count
